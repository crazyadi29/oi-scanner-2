...
def _load_token() -> tuple[str, str]:
    env_token = os.getenv("FYERS_ACCESS_TOKEN_2", "").strip()
    if env_token:
        if ":" in env_token:
            client_id, access_token = env_token.split(":", 1)
            return client_id.strip(), access_token.strip()
        client_id = os.getenv("FYERS_CLIENT_ID_2", "").strip()
        if not client_id:
            try:
                import config
                ...
                pass
        if not client_id:
            raise RuntimeError("FYERS_ACCESS_TOKEN_2 missing CLIENT_ID prefix.")
    ...
    try:
        ...
        raise RuntimeError("No Fyers token found for bot2.")
...
class NSEClient:
    ...
    def _refresh_if_needed(self):
        current = os.getenv("FYERS_ACCESS_TOKEN_2", "").strip()
