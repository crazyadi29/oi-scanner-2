...
def delete_webhook(token: str):
    """Remove any webhook set by spammers."""
    try:
...
def block_incoming(token: str, allowed_chat_id: str):
    """
    Poll getUpdates and delete any message not from allowed_chat_id.
    Call once at startup to clear spam.
    """
    try:
        ...
        for update in data.get("result", []):
            ...
            if str(chat.get("id", "")) != str(allowed_chat_id):
                log.warning(f"Blocked message from unauthorized chat: {chat.get('id')}")
    except Exception as e:
        log.error(f"block_incoming failed: {e}")
